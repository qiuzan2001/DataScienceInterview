...
* **Pros:**
  ...
  * **Mainly reduces bias:** each round fits the residual / pseudo-residual errors of the current ensemble, so systematic error (bias) drops fast. **The effect on variance depends on regularization and early stopping:** with a small learning rate + shallow trees + early stopping, variance usually stays controlled or even decreases; with too many rounds and no regularization, variance rises.
...
## [[8.6 Bagging VS Boosting| 6. Choosing the Right Ensemble Method]]

| Aspect                 | Bagging                                        | Boosting                                             | Stacking                                               |
| ---------------------- | ---------------------------------------------- | ---------------------------------------------------- | ------------------------------------------------------ |
| **Bias Reduction**     | No                                             | Yes                                                  | Depends on meta-learner                                |
| **Variance Reduction** | Yes                                            | Conditional: small learning rate + shallow trees + early stopping → controlled or reduced; too many unregularized rounds → rises (see §4.4) | Yes                                                    |
...
3. **Boosting** (sequential, error-focused) **mainly reduces bias**; its effect on variance depends on regularization and early stopping — small learning rate + shallow trees + early stopping keeps variance controlled (often lower), while too many unregularized rounds push it up (the cost is potential overfitting and complexity).
