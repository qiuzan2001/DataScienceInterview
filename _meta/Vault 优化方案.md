...
按 §3 的批次全部执行完毕，另有超出原计划的产出（题库、保险章、核验报告）。

| Batch | 内容 | 状态 |
|---|---|---|
| 0 | 基线 + 推送 v1.0.0-baseline + Release | ✅ |
| 1 | P0 硬伤：空文件、WOE 三处冲突、AdaBoost 算例、Boosting 方差、5 处断章、8 处 AI 残留、渲染、canvas 死链 | ✅ |
| 2 | 冲刺层（00–05）、全库 frontmatter、优先级标记、根笔记降级为速查卡 | ✅ |
| 3 | 09→**10 章保险风控实务**（8 篇 1678 行）+ SAS 代码规范 | ✅ |
| 4 | **99 题库**：72 题，含正确答案 / 错误答案 / 追问链 | ✅ |
| — | `_meta/事实核验报告.md`：用 PSL / Loss Data Analytics 逐条核验 | ✅ 超出计划 |
...
| 双井号标题 | 2 | **0** |
| `\$` 转义残留 | 4 | **0** |
| 笔记数 / 行数 | 42 / 8146 | 64 / 13935 |

### 未做的部分（有意保留）

- **编号跳号**（2.1–2.2、3.1–3.3、5.1、7.1、7.3、8.1–8.2、8.5 缺位）：属「父笔记 + 按需子笔记」的正常结构，重编号会改动大量链接，收益低于风险，故保留。
- **State Farm 专属情报**：Glassdoor 403 / Reddit 拦截 / iCIMS 为 JS 渲染，未能取得真题与 JD，相关笔记已明确标注「未获取到」，未编造。
